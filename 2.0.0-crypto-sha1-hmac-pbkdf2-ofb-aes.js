/*
 * Krypto-JS v2.0.0
 * http://code.google.com/p/Krypto-js/
 * Copyright (c) 2009, Jeff Mott. All rights reserved.
 * http://code.google.com/p/Krypto-js/wiki/License
 */
(function() {
...
(function() {
    ...
    var c = [[], [], [], []], d, b, t;
    var o = l.AES = {
        encrypt: function(A, z, y) {
            var i = s.stringToBytes(A)
              , x = a.randomBytes(o._blocksize * 4)
            ...
            o._init(w);
            mode.encrypt(o, i, x);
            return a.bytesToBase64(x.concat(i))
        },
        decrypt: function(z, y, x) {
            var A = a.base64ToBytes(z)
              , w = A.splice(0, o._blocksize * 4)
            ...
            o._init(i);
            mode.decrypt(o, A, w);
            return s.bytesToString(A)
        },
        _blocksize: 4,
        _encryptblock: function(w, x) {
            for (var D = 0; D < o._blocksize; D++) {
                for (var i = 0; i < 4; i++) {
                    c[D][i] = w[x + i * 4 + D]
                }
            }
        ...
        _keyexpansion: function(w) {
            t = [];
            for (var x = 0; x < d; x++) {
                t[x] = [w[x * 4], w[x * 4 + 1], w[x * 4 + 2], w[x * 4 + 3]]
            }
            for (var x = d; x < o._blocksize * (b + 1); x++) {
